...
class FirstScreen: Navbar {
    ...
    private let Hobbies = ["Spor","Gezi","Sanat","Film","Tenis","Golf","Araç","Oyun Oynamak","Teknoloji","Sanat","Müzik"]
...
extension FirstScreen : SetUpViews{
    func setupViews() {
        navbarLabel = {
            ...
            navbarLAbel.text = "Hobileriniz ile başlayalım!!"
            return navbarLAbel
        }()
        navbarButton = {
           let button = UIButton()
    ...
    func setupFrameWithPhone(withdeviceName: PhoneType) {
        ...
        if deviceModel == .iPhoneSE{
            navbarLabel.frame = CGRect(x: (screenWith  / 2 ) - 120, y: 40, width: 200, height: 40)
            navbarButton.frame = CGRect(x: (screenWith  / 2 ) + 80, y: 40, width: 80, height: 40)
        }else {
            navbarLabel.frame = CGRect(x: (screenWith  / 2 ) - 100, y: 40, width: 200, height: 40)
            navbarButton.frame = CGRect(x: (screenWith  / 2 ) + 120, y: 40, width: 80, height: 40)
        }
    ...
    @objc func saveButtonTapped(){
        ...
        if selectedList.count == 0{
           self.present( CreateAlert.referance.createAlert(withTitle: "Bilgi", andMessage: "Hobilerinizi seçin", andActionTitle: "Tamam"),animated: true,completion: nil)
        }else if selectedList.count < 3 {
            self.present( CreateAlert.referance.createAlert(withTitle: "Bilgi", andMessage: "En az 3 tane seçmelisiniz.", andActionTitle: "Tamam"),animated: true,completion: nil)
        }
